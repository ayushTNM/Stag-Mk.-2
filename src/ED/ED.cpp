...
EdgeMap *DetectEdgesByCannySR(unsigned char *srcImg, int width, int height, int cannyLowThresh, int cannyHighThresh, int sobelKernelApertureSize, double smoothingSigma){
  ...
  // } //end-for
  cv::Mat iplImg = cv::cvarrToMat(srcImg).clone();
  cv::Mat edgeImg;
  if (smoothingSigma <= 1.0) cv::GaussianBlur(iplImg, iplImg, cv::Size(5, 5),0);
  else cv::GaussianBlur(iplImg, iplImg, cv::Size(0, 0),smoothingSigma);
  // iplImg = IplImage( iplMat );
  ...
  for (int i=1; i<height-1; i++){
    unsigned char *p = (unsigned char *)(edgeImg.data + edgeImg.step*i);
  ...
  cv::GaussianBlur(edgeImg, edgeImg, cv::Size(5, 5),0);
//  cvSmooth(edgeImg, edgeImg, CV_GAUSSIAN, 0, 0, 0.5);
  ...
  for (int i=1; i<height-1; i++){
    ...
    for (int j=1; j<width-1; j++){
      ...
      int com1 = smoothImg.data[(i+1)*width+j+1] - smoothImg.data[(i-1)*width+j-1];
      int com2 = smoothImg.data[(i-1)*width+j+1] - smoothImg.data[(i+1)*width+j-1];

      int gx = abs(com1 + com2 + (smoothImg.data[i*width+j+1] - smoothImg.data[i*width+j-1]));
      int gy = abs(com1 - com2 + (smoothImg.data[(i+1)*width+j] - smoothImg.data[(i-1)*width+j]));
      int sum = gx + gy;
      int index = i*width+j;
      gradImg[index] = sum;
