...
void SmoothImage(unsigned char *srcImg, unsigned char *smoothImg, int width, int height, double sigma){
  ...
  iplImg1 = cv::Mat(height, width, CV_8UC1, srcImg);
  // iplImg1.data = (unsigned char *)srcImg;
  // iplImg1.step = width;

  // iplImg2 = cvCreateImageHeader(cvSize(width, height),IPL_DEPTH_8U, 1);
  iplImg2 = cv::Mat(height, width, CV_8UC1, smoothImg);
  // iplImg2.step = width;

  if (sigma == 1.0)
    // cvSmooth(iplImg1, iplImg2, CV_GAUSSIAN, 5, 5);
    cv::GaussianBlur(iplImg1, iplImg2, cv::Size(5, 5),0);
  else if (sigma == 1.5)
    // cvSmooth(iplImg1, iplImg2, CV_GAUSSIAN, 7, 7);  // seems to be better?
    cv::GaussianBlur(iplImg1, iplImg2, cv::Size(7, 7),0);
  else 
    // cvSmooth(iplImg1, iplImg2, CV_GAUSSIAN, 0, 0, sigma);
    cv::GaussianBlur(iplImg1, iplImg2, cv::Size(0, 0),sigma);
...
/// If sigma>0.0, then calls cvSmooth(srcImg, smoothedImg, CV_GAUSSIAN, 0, 0, sigma);
///
void SmoothImage(IplImage *iplImg11, unsigned char *smoothImg, double sigma){

  cv::Mat iplImg2;
  cv::Mat iplImg1 = cv::cvarrToMat(iplImg11);

  if (sigma <= 0){
    // CopyFromIplBuffer(smoothImg, iplImg1);
    smoothImg = iplImg1.data;
    return;
  } //end-if
